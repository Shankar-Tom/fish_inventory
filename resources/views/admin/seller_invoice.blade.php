...
<body>
    ...
    <div class="container">
        ...
        <div class="body-section">
            <div class="row">
                <div class="col-6">
                    <h2 class="heading">Invoice No.: INV-PUR-{{$bill->id}}</h2>
                    <p class="sub-heading">Date : {{$bill->created_at->format('d M Y')}} </p>
                </div>
                <div class="col-6">
                    <p class="sub-heading">Full Name: {{$bill->Billable->party_name}} </p>
                    <p class="sub-heading">Port: {{$bill->Billable->Port->port_name}}  </p>
            ...
            <table class="table-bordered">
                ...
                <tbody>
					...
					@foreach ($purchases as $purchase)
						@php($purchasedetail=\App\Models\Purchase::find($purchase->purchase_sale_id))
					

						<tr>
							<td>{{$purchasedetail->created_at->format('d M Y')}}</td>
							<td>{{$purchasedetail->product }} - {{$purchasedetail->Category->name}}</td>
